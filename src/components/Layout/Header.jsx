...
const Header = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 p-6 md:p-8 flex justify-between items-start z-50 pointer-events-none">
      ...
      <div className="absolute left-1/2 -translate-x-1/2 top-6 md:hidden flex gap-4 pointer-events-auto z-[60]">
        {/* Resume Toggle (Blue) */}
        <button
          onClick={() => {
          ...
          {(isResumeOpen || window.innerWidth >= 768) && (
            <motion.div
              ...
              className={`
                    ...
                    ${isResumeOpen ? 'fixed top-24 left-4 right-4 mx-auto w-auto max-w-sm z-[70] shadow-2xl md:static md:w-72 md:mx-0 md:shadow-sm' : 'hidden md:flex'}
                `}
            >
          ...
          {(isWidgetOpen || window.innerWidth >= 768) && (
            <motion.div
              ...
              className={`
                                ...
                                ${isWidgetOpen ? 'fixed top-24 left-4 right-4 mx-auto w-auto max-w-sm z-[70] shadow-2xl md:static md:w-72 md:mx-0 md:shadow-sm' : 'hidden md:flex'}
                            `}
            >
